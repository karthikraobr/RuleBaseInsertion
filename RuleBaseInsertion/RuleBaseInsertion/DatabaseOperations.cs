...
namespace RuleBaseInsertion
{
    ...
    {
        string connectionString = @"Data Source=.;Initial Catalog=Sage.FA.DaaS.Master;User ID=sa;Password=admin@123";
        ...
        public Guid GetS179ClassificationBasedOnCode(string code)
        {
            Guid s179ClassificationtId = Guid.Empty;
            string selectQuery = "SELECT [S179ClassificationID] FROM [S179Classification] where [Code]='{0}'";
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                con.Close();
                s179ClassificationtId = Guid.Parse(guid.ToString());
            }
            return s179ClassificationtId;
        }
        ...
        public Guid AddRuleHeader(Guid valueIdentifier, Guid rulePropertyId, string effectiveDate, string expirationDate, string parentRuleHeaderId, int ruleType)
        {
           ...
           "Values" +
            ...
            insertQuery = string.Format(insertQuery,
                ...
                valueIdentifier,
                rulePropertyId,
                !string.IsNullOrEmpty(effectiveDate) ? "'" + Convert.ToDateTime(effectiveDate,CultureInfo.InvariantCulture).ToShortDateString() + "'" : "null",
                !string.IsNullOrEmpty(expirationDate) ? "'" + Convert.ToDateTime(expirationDate, CultureInfo.InvariantCulture).ToShortDateString() + "'" : "null",
        ...
        public Guid GetRuleHeaderIdBasedOnDate(string rulepropertyid, string bookId, string effectiveDate, string expirationdate, string defRowHeaderId)
        {
            ...
            if (string.IsNullOrEmpty(expirationdate))
            {
                ...
                selectQuery = string.Format(selectQuery, rulepropertyid, bookId, !string.IsNullOrEmpty(effectiveDate) ? "'" + Convert.ToDateTime(effectiveDate).ToShortDateString() + "'" : "is null",
                !string.IsNullOrEmpty(expirationdate) ? "'" + Convert.ToDateTime(expirationdate).ToShortDateString() + "'" : "is null", defRowHeaderId);
            }
            ...
            {
                selectQuery = "SELECT [ruleheaderid] FROM [ruleheader] where[rulepropertyid]='{0}' and [valueidentifier]='{1}' and [effectivedate]= {2} and [expirationdate]={3} and [parentruleheaderid] ='{4}'";
                selectQuery = string.Format(selectQuery, rulepropertyid, bookId, !string.IsNullOrEmpty(effectiveDate) ? "'" + Convert.ToDateTime(effectiveDate).ToShortDateString() + "'" : "null",
                    !string.IsNullOrEmpty(expirationdate) ? "'" + Convert.ToDateTime(expirationdate).ToShortDateString() + "'" : "null", defRowHeaderId);
            }
        ...
        {
            string selectQuery = "SELECT [BookTypeID] FROM [BookType] where [CODE]='{0}' ";
            selectQuery = string.Format(selectQuery, code);
