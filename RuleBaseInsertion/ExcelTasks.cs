...
namespace RuleBaseInsertion
{
    public class ExcelTasks
    {
        ...
        public List<string[]> ConvertExcelToListMacrsBonus(Worksheet workSheet)
        {
            ...
            for (int i = 7; i <= 50; i++)
            {
                Range range = workSheet.get_Range("C" + i.ToString(), "AC" + i.ToString());
                System.Array myvalues = (System.Array)range.Cells.Value;
                string[] strArray = ConvertToStringArray(myvalues);
                rows.Add(strArray);
            }
        ...
        public List<string[]> ConvertExcelToListITC(Worksheet workSheet)
        {
            ...
            for (int i = 2; i <= 23; i++)
            {
                Range range = workSheet.get_Range("B" + i.ToString(), "AC" + i.ToString());
                System.Array myvalues = (System.Array)range.Cells.Value;
                string[] strArray = ConvertToStringArray(myvalues);
                rows.Add(strArray);
            }
        ...
        public List<string[]> ConvertExcelToListS179(Worksheet workSheet)
        {
            ...
            for (int i = 4; i <= 110; i++)
            {
                Range range = workSheet.get_Range("A" + i.ToString(), "AE" + i.ToString());
                System.Array myvalues = (System.Array)range.Cells.Value;
                string[] strArray = ConvertToStringArray(myvalues);
                rows.Add(strArray);
            }
